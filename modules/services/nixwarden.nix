{
  nixos.services.nixwarden =
    ...
    with lib;
    let
      # TODO: Not sure I like the "olistrik" namespace.
    ...
    in
    {
      ...
      config =
        let
          ...
          writeSecret =
            name: file: "writeSecret ${name} ${file.location} ${file.userGroup} ${file.permissions}";
          ...
          writeSecrets = concatStringsSep "\n" (
            concatLists (mapAttrsToList (name: files: (map (file: (writeSecret name file)) files)) cfg.secrets)
          );
        in
        mkIf (length keys > 0) {
          assertions = [
